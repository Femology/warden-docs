...
{% hint style="warning" %}
This contract has no admin-upgrade function by design, so it's been redeployed a few
  times as new features shipped. Earlier instances are retired; the ID above is
  current. See `warden-contract`'s README for the full deployment history.
{% endhint %}
...
pub enum StepUpReason {
    ...
    FlaggedRecipient, // see WARDEN-PROTOCOL.md
}
```
...
pub enum WardenError {
    ...
    // 8 through 20 cover the flagged-address registry and the
